...
int main(void)
{
	...
	//Key constants
	const char IP_SERV[] = "0.0.0.0";			// Enter local Server IP address
	...
	while (true)
	{
		...
		while (true) {
			vector <char> servBuff(BUFF_SIZE);							// Creation of buffers for sending and receiving data
			...
			// Checking whether the data has been received. If there is no data, it means that program No. 1 is disabled. Waiting for connection.
			...
			float Sum = 0;		// The number received from program #1

			for (int i = int(servBuff[0]); i > 0; i--)
			{
				Sum *= 10;
				Sum += servBuff[i] - 48;
			}
			...
			// 
			if (servBuff[0] > 2 && (Sum / 32 == round(Sum / 32)))
				cout << "The data (" << Sum << ") was received";
			else
				cout << "Error! (" << Sum << ")";
